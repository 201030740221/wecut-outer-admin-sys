...
var TagIndex = React.createClass({
  ...
  fetch(params = {}) {
    ...
    reqwest({
      url: apiConfig.apiHost+'/cms/tag/channel/list.php',
      method: 'get',
      data: params,
      type: 'json',
      success: (result) => {
  ...
  tagClick(chid,tagid){
    ...
    reqwest({
      url: apiConfig.apiHost+'/cms/tag/channel/add.php',
      method: 'post',
      data: params,
      type: 'json',
      success: (result) => {
  ...
  delTag(chid,tagid){
    ...
    reqwest({
      url: apiConfig.apiHost+'/cms/tag/channel/del.php',
      method: 'post',
      data: params,
      type: 'json',
      success: (result) => {
  				...
  				<Row>
  					<Col span="4">
  						<div className="u-p-10">
  							<img src={record.coverimage} className="header_pic" />
  							<p className="mt-10">频道名：{record.name}</p>
  							<p className="mt-15">分类：{record.type}</p>
  						</div>
  					</Col>
  					<Col span="20">
  						<div className="u-p-10">
                   ...
                   <Col span="10">
                      <div className="">
                        <p className="">频道介绍：{record.sintro?record.sintro:'暂无介绍'}</p>
                      </div>
                   </Col>
  								 <Col span="1">
		  	  						.
		  	  							...
		  	  							<span className="ml-5">
                          {
                            record.existedTags.map(function(item,key){
                                ...
                                return (
                                  <Tag key={key} closable onClose={self.delTag.bind(null,record.chid,item.tagid)}>{item.name}</Tag>
                                )
                            })
                          }
		  	  							</span>
  	  								...
  	  								<span className="title">一级：</span>
                      {
                        record.firstLevelTag.map(function(_item,index){
                          return (
                            <span key={index} className={"tag_list "+_item.tagStatus} onClick={self.tagClick.bind(null,record.chid,_item.tagid)}>{_item.name}</span>  
                          )
                        })
                      }
  	  								...
  	  								<span className="title">二级：</span>
                      {
                        record.secondLevelTag.map(function(_item,index){
                          if(_item){
                              return (
                                <span key={index} className={"tag_list "+_item.tagStatus} onClick={self.tagSecondClick.bind(null,record.chid,_item.id)}>{_item.name}</span>  
                              )
                            }else{
                              return(
                                <span>暂无</span>
